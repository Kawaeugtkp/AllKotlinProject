        ...
        Card(
            ...
            Image(
                painter = painterResource(
                    id = jp.gardenall.instagramclone.R.drawable.ic_add
                ),
...
fun PostList(
    ...
    if (postsLoading) {
        ...
        LazyColumn(modifier = modifier) {
            ...
            // とりあえずここでcurrentRowをrowsに入れておく（入れているけど、この入れたcurrentRowの中身は、
            // 入れた後でもcurrentRow自体の編集によって変更できる）
            rows.add(currentRow)
            for (post in posts) {
            ...
            items(items = rows) { row ->
                PostsRow(items = row, onPostClick = onPostClick)
            }
        }
    }
}
...
@Composable
fun PostsRow(items: PostRow, onPostClick: (PostData) -> Unit) {
    Row(
        modifier = Modifier
            .fillMaxWidth()
            .height(120.dp)
        ...
        PostImage(
            imageUrl = items.post1?.postImage,
            modifier = Modifier
                .weight(1f)
                .clickable { items.post1?.let { post -> onPostClick(post) } }
        )
        PostImage(
            imageUrl = items.post2?.postImage,
            modifier = Modifier
                .weight(1f)
                .clickable { items.post2?.let { post -> onPostClick(post) } }
        )
        PostImage(
            imageUrl = items.post3?.postImage,
            modifier = Modifier
                .weight(1f)
                .clickable { items.post3?.let { post -> onPostClick(post) } }
        )
    }
}
